<?php
require $_SERVER['DOCUMENT_ROOT'] . '/soap-wsse.php';
require $_SERVER['DOCUMENT_ROOT'] . '/JWT.php';
...
class ET_Client extends SoapClient {
    ...
    function CreateWSDL($wsdlLoc) {
        ...
        try{
            ...
            if (file_exists($_SERVER['DOCUMENT_ROOT'] . "/cache/ExactTargetWSDL.xml")){
                $localTS = filemtime($_SERVER['DOCUMENT_ROOT'] . "/cache/ExactTargetWSDL.xml");
                if ($remoteTS <= $localTS) 
                {
                    $getNewWSDL = false;
                }
            ...
            if ($getNewWSDL){
                ...
                file_put_contents($_SERVER['DOCUMENT_ROOT'] . "/cache/ExactTargetWSDL.xml", $newWSDL);
            }   
        }
        catch (Exception $e) {
    ...
    function AddSubscriberToList($emailAddress, $listIDs, $subscriberKey = null){                   
        ...
        if ($postResponse->status == false) {
            // If the subscriber already exists in the account then we
...
class ET_Get extends ET_Constructor {
    function __construct($authStub, $objType, $props, $filter, $getSinceLastBatch = false) {
        ...
        // If Props is not sent then Info will be used to find all
        ...
        if ($this->status){
            if (property_exists($return, "Results")){
                // We always want the results property when doing a
                // retrieve to be an array
...
class ET_Import extends ET_CUDSupport {
    ...
    function post() {
        ...
        # If the ID property is specified for the destination then it
        # must be a list import
...
function restGet($url) {
    ...
    // Need to set ReturnTransfer to True in order to store the result
    // in a variable
 ...
 * @param string      $content    A string of JSON which will be
...
function restPost($url, $content) {
    ...
    // When posting to a Fuel API, content-type has to be explicitly
