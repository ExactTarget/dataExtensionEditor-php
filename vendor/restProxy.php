...
//
// Modified to match semantics of FuelAPIs Ajax Proxy as part of the 
// FuelAPI Family JavaScript REST Toolkit to allow JavaScript outside the
// platform to use the Fuel SDK PHP.
// 
// About: Examples
// 
// This working example, complete with fully commented code,
// illustrates one way
...
// 
// 2.0   (4/15/2011) Modified to work like the Force.com Ajax Proxy so
// that
//       JavaScript apps can access the Force.com REST API from
//       outside the
//       platform.
// 1.6 - (1/24/2009) Now defaults to JSON mode, which can now be
// changed to
//       native mode by specifying ?mode=native. Native and JSONP
//       modes are
//       disabled by default because of possible XSS vulnerability
//       issues, but
//       are configurable in the PHP script along with a url
//       validation regex.
...
// 
//   url - The remote URL resource to fetch. Any GET parameters to be
//   passed
//     through to the remote URL resource must be urlencoded in this
//     parameter.
//   mode - If mode=native, the response will be sent using the same
//   content
//     type and headers that the remote URL resource returned. If
//     omitted, the
//     response will be JSON (or JSONP). <Native requests> and <JSONP
//     requests>
//     are disabled by default, see <Configuration Options> for more
//     information.
//   callback - If specified, the response JSON will be wrapped in
//   this named
//     function call. This parameter and <JSONP requests> are disabled
//     by
//     default, see <Configuration Options> for more information.
//   user_agent - This value will be sent to the remote URL request as
//   the
//     `User-Agent:` HTTP request header. If omitted, the browser user
//     agent
//     will be passed through.
//   send_cookies - If send_cookies=1, all cookies will be forwarded
//   through to
//     the remote URL request.
//   send_session - If send_session=1 and send_cookies=1, the SID
//   cookie will be
//     forwarded through to the remote URL request.
//   full_headers - If a JSON request and full_headers=1, the JSON
//   response will
//     contain detailed header information.
//   full_status - If a JSON request and full_status=1, the JSON
//   response will
//     contain detailed cURL status information, otherwise it will
//     just contain
//     the `http_code` property.
// 
// Topic: POST Parameters
// 
// All POST parameters are automatically passed through to the remote
// URL
// request.
// 
// Topic: JSON requests
// 
// This request will return the contents of the specified url in JSON
// format.
// 
// Request:
// 
...
//   headers - (Object) A hash of HTTP headers returned by the remote
//   URL
//     resource.
...
// 
// This request will return the contents of the specified url in JSONP
// format
...
// 
// This request will return the contents of the specified url in the
// format it
// was received in, including the same content-type and other headers
// (but only
...
// 
// These variables can be manually edited in the PHP file if
// necessary.
// 
//   $enable_jsonp - Only enable <JSONP requests> if you really need
//   to. If you
//     install this script on the same server as the page you're
//     calling it
//     from, plain JSON will work. Defaults to false.
//   $enable_native - You can enable <Native requests>, but you should
//   only do
//     this if you also whitelist specific URLs using
//     $valid_url_regex, to avoid
//     possible XSS vulnerabilities. Defaults to false.
//     Force.com JavaScript REST Toolkit sets this to true, since it
//     needs
//     native mode, and sets $valid_url_regex appropriately.
//   $valid_url_regex - This regex is matched against the url
//   parameter to
//     ensure that it is valid. This setting only needs to be used if
//     either
//     $enable_jsonp or $enable_native are enabled. Defaults to '/.*/'
//     which
//     validates all URLs.
//     Force.com JavaScript REST Toolkit sets this to 
//     '/https:\/\/.*salesforce.com/' to constrain URLs to end with 
//     salesforce.com
//   $url_query_param - set this to the name of the query string
//   parameter in
//     which to pass the target URL, or null if you want to pass the
//     URL as a
//     header.
//   $url_header - set this to the name of the HTTP header in which to
//   pass 
//     the target URL, or null if you want to pass the URL as a query
//     string
//     parameter.
//     Force.com JavaScript REST Toolkit sets this to 
//     'HTTP_SALESFORCEPROXY_ENDPOINT' to match the Ajax proxy.
//   $authz_header - HTTP header in which authorization data is
//   passed. This
//     will be used for the outgoing Authorization header.
//     Force.com JavaScript REST Toolkit sets this to 'AUTHORIZATION'
//   $cors_allow_origin - origins to allow for Cross Origin Resource
//   Sharing.
//     Set this to match the location of the JavaScript - e.g. 
//     'https://www.myserver.com/'
//   $cors_allow_methods - HTTP methods to allow for CORS - you
//   shouldn't need
//     to change this.
//   $cors_allow_headers - HTTP headers to allow for CORS - you might
//   need to
...
$enable_jsonp    = false;
$enable_native   = true;
$valid_fuelapi_url_regex = '/https:\/\/.(www|qa1s1|qa2s1|qa2s2|qa3s1)\.exacttargetapis\.com/';


$url_query_param = null; // 'url'
$url_header = 'HTTP_FUELAPIPROXY_ENDPOINT';
...
} else if ( !preg_match( $valid_fuelapi_url_regex, $url )) {
...
if ( isset($_GET['mode']) && $_GET['mode'] == 'native' ) {
  ...
  // Propagate all cURL request / response info to the JSON data
  // object.
  ...
  // Set the JSON data object contents, decoding it from JSON if
  // possible.
